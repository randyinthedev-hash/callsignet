...
직통 경로를 닫는 것 다섯을 본다. Rocky에서는 운영자가 할 일 둘을 그대로 한다. 클라우드 이미지에 `nftables` 패키지가 없으므로 설치하고, firewalld를 끄지 않은 채 wg 포트만 연다. firewalld의 표 곁에 csa의 표가 함께 도는지, 이 호스트가 csa 없는 머신 노릇을 해서 실제 IP로 서비스 포트에 붙지 못하는지, 적지 않은 포트는 그대로 열려 있는지 본다.
