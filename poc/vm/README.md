...
직통 경로를 닫는 것 다섯을 본다. Rocky에서 firewalld를 끄지 않고 wg 포트만 연다. 실제 배포에서 운영자가 할 일이 그것이다. firewalld의 표 곁에 csa의 표가 함께 도는지, 이 호스트가 csa 없는 머신 노릇을 해서 실제 IP로 서비스 포트에 붙지 못하는지, 적지 않은 포트는 그대로 열려 있는지 본다.
