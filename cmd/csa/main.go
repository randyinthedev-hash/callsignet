...
func reload(dir string, live *atomic.Pointer[config.Config], dev device,
	...
	if !ch.Any() {
		// 뜻은 같아도 파일은 새것이다. 상대의 순서만 다르거나 주석과 빈 줄만
		// 다른 파일 셋이 그렇다. wg와 규칙과 표는 두 설정이 같은 것을 걸므로
		// 건드리지 않고, 따르는 설정만 지금 읽은 것으로 바꾼다. 그래야 csa status의
		// 설정 해시가 이 파일 셋을 가리킨다. 설정을 두는 쪽은 그 해시로 csa가
		// 자기가 둔 파일을 물고 있는지 확인하기 때문이다.
		live.Store(cur)
		logf("설정을 다시 읽었습니다. 바뀐 것이 없습니다. 지금 읽은 파일을 따르는 설정으로 삼았습니다.")
		return "바뀐 것이 없습니다. 지금 읽은 파일을 따르는 설정으로 삼았습니다.\n", nil
	}
