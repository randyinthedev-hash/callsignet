...
6. 답하지 않으면 **스스로 앞 판으로 되돌린다.** 링크를 되돌리고 다시 띄우고 답하는지 다시 본다. 무엇이 잘못됐는지는 `journalctl -u csa`에 남는다.
...
`current`와 `previous`를 맞바꾸고, 서비스 파일을 앞 판의 것으로 되돌리고, 돌고 있으면 다시 띄우고 답하는지 본다. 앞 판의 디렉터리가 그대로 있으므로 내려받을 것이 없다.
